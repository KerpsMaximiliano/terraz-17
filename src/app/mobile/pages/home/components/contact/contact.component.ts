...
export class ContactComponent {
  ...
  public onSubmit(): void {
    if (this.form.invalid) {
      ...
      this.sendEmail({
        message: {
          html: `Teléfono: ${this.form.get('phone')?.value}<br>
                 Correo: ${this.form.get('mail')?.value}<br>
  ...
  private _setForm(): UntypedFormGroup {
    return new UntypedFormGroup({
      name: new UntypedFormControl(
        null,
        Validators.compose([Validators.required,Validators.minLength(3), Validators.maxLength(30),  isLettersAndSpacesOnly(), notOnlySpaces()])
      ),
